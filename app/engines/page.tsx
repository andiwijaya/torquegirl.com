...
export default function EnginesPage() {
  return <main className="site-shell article-shell">
    ...
    <section className="article-index" aria-labelledby="latest-engines"><div className="article-index-heading"><p className="eyebrow"><span className="eyebrow-line" />Latest from the garage</p><h2 id="latest-engines">Power, decoded.</h2></div><div className="article-card-grid">{articles.map((article) => <a className="article-card" href={`/engines/${article.slug}/`} key={article.slug}><div className="article-card-image"><img src={article.heroImage} alt="" loading="lazy" /></div><div className="article-card-copy"><div className="article-card-meta"><span>{article.category}</span><span>{article.readingTime}</span></div><h3>{article.title}</h3><p>{article.description}</p><span className="text-link">Read the article <ArrowUpRight size={16} /></span></div></a>)}</div></section>
