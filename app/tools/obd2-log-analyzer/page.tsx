...
export default function AnalyzerPage() {
  return <main className="obd-page"><header className="obd-header"><Link className="brand" href="/" aria-label="TorqueGirl home"><span className="brand-mark">T</span><span>Torque<span>Girl</span><b>.com</b></span></Link><nav aria-label="Main navigation"><Link href="/technology">Technology</Link><Link href="/tools" aria-current="page">Tools</Link></nav><span className="obd-local-badge">LOCAL / PRIVATE</span></header>
    <section className="obd-hero"><div><p className="eyebrow"><span className="eyebrow-line" />TorqueGirl / Data tools / V2</p><h1>Power, <em>recorded.</em><br />Now make sense of it.</h1><p>OBD2 LOG ANALYZER</p></div><div className="obd-hero-note"><span>THE SIGNAL IS THE STORY.</span><p>Understand your log. Inspect a moment. Follow the relationship between sensors.</p><small>Recorded data and transparent observations. No automated diagnosis.</small></div></section>
    <Analyzer />
    <section className="obd-learn"><h2>Data first. Context always.</h2><p>Use a CSV or TSV with one header row and one row per sample. Time (s), Time (ms), ISO date/time and HH:MM:SS are supported. Put units in parentheses or brackets. Some Torque, OBD Fusion and FORScan exports fit this format; proprietary files and every exporter variant are not supported.</p><p><Link href="/technology/what-is-an-obd2-scanner">What an OBD2 scanner tells you ↗</Link><Link href="/technology/how-to-read-obd2-codes">Understanding diagnostic codes ↗</Link></p></section>
