...
function Chart({ signal, data, reading, color, range, cursor, onInspect }: { signal: SignalInfo; data?: Trace; reading?: Reading; color: string; range: [number, number]; cursor: number; onInspect: (time: number) => void }) {
  ...
  return <section className="obd-chart" aria-label={`${signal.originalName} chart`}>
    ...
    <svg ref={svgRef} viewBox={`0 0 ${width} 158`} role="slider" tabIndex={0} aria-label={`Inspect ${signal.originalName}`} aria-valuemin={range[0]} aria-valuemax={range[1]} aria-valuenow={Math.max(range[0], Math.min(range[1], cursor))} aria-valuetext={clock(cursor)} onKeyDown={e => { if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') { e.preventDefault(); onInspect(Math.max(range[0], Math.min(range[1], cursor + (e.key === 'ArrowRight' ? 1 : -1) * span / 100))); } }} onPointerDown={e => { e.currentTarget.setPointerCapture(e.pointerId); pointer(e); }} onPointerMove={e => { if (e.buttons) pointer(e); }}>
      ...
      {(width < 420 ? [0, 0.5, 1] : [0, 0.25, 0.5, 0.75, 1]).map(f => <text key={f} x={left + f * (right - left)} y="151" textAnchor="middle">{clock(range[0] + f * span)}</text>)}
    </svg>
...
export default function Analyzer() {
  ...
  const load = async (file: Blob, label: string) => {
    ...
    setPlaying(false); setBusy(true); setError(''); setInfo(null); setTraces([]); setReadings([]); setCursor(0); setName(label);
    let next: LogWorkerClient | null = null;
    try {
      next = new LogWorkerClient(new ObdWorker());
      client.current = next;
      const response = await next.request({ kind: 'load', file });
      if (client.current !== next || !('kind' in response) || response.kind !== 'load') return;
      setInfo(response.info); setRange([0, response.info.quality.duration]);
  ...
  return <div className="obd-app">
    <section className={`obd-import ${info ? 'obd-import-compact' : ''}`} aria-label="Import a log">
      <div><span className="obd-kicker">01 / IMPORT</span><h2>{info ? name : 'Every drive tells a story.'}</h2><p>{info ? `${info.format} · ${number(q?.parsed)} records · ${clock(duration)}` : 'Bring your recorded data. Explore what the sensors actually saw.'}</p></div>
      <div className="obd-import-actions"><label className="obd-button obd-primary">{busy ? 'Replace file' : info ? 'Open another log' : 'Choose a CSV log'}<input aria-label="Choose CSV log" type="file" accept=".csv,.tsv,.txt,text/csv,text/tab-separated-values" onChange={e => { const f = e.target.files?.[0]; if (f) void load(f, f.name); e.target.value = ''; }} /></label><button onClick={() => void load(new Blob([demoCsv()], { type: 'text/csv' }), 'Synthetic demo · illustrative data')}>Explore demo</button></div>
      {!info && <p className="obd-import-note">CSV / TSV · up to 25 MiB, 250,000 records, 2 million cells · comma, semicolon or tab</p>}
    </section>
    <p className="obd-privacy">◉ Local processing. Your log stays in this browser. Nothing is uploaded or saved after you leave.</p>
    {busy && <div className="obd-notice" role="status">Reading and normalizing your log in the background… <button onClick={() => { client.current?.dispose(); client.current = null; setBusy(false); }}>Cancel import</button></div>}
    {error && <p className="obd-error" role="alert">{error}</p>}
    {!info && !busy && <div className="obd-empty-grid"><section><span>02 / UNDERSTAND</span><h3>Trust the timeline.</h3><p>See missing samples, recording gaps and uncertain units before reading a chart.</p></section><section><span>03 / INSPECT</span><h3>What happened here?</h3><p>Tap a moment to see synchronized readings, their source times and how old they are.</p></section><section><span>04 / EXPLORE</span><h3>Follow the relationships.</h3><p>Replay real elapsed time. Compare RPM, throttle and the signals your file contains.</p></section></div>}
    {info && q && <>
      ...
      <details className="obd-quality"><summary>Import quality & source mapping <span>{q.warningCount} notices · {q.rejected} rejected records</span></summary>
        ...
        <div className="obd-table-scroll"><table><caption>Original columns → canonical signals</caption><thead><tr><th>Original column</th><th>Identity</th><th>Source → display unit</th><th>Missing / nonnumeric</th></tr></thead><tbody>{info.signals.map(s => <tr key={s.id}><td>{s.originalName}<small>{s.id}</small></td><td>{s.identity ?? 'Custom / unknown'}</td><td>{s.originalUnit ?? 'unknown'} → {s.unit ?? 'unknown'}{s.ambiguity && <small>{s.ambiguity}</small>}</td><td>{s.missing} / {s.invalid}</td></tr>)}</tbody></table></div>
      </details>
