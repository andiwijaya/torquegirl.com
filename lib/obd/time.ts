...
export function parseTime(raw: string, kind: ReturnType<typeof timeKind> | 'clock' | 'iso', decimal: boolean): number | null {
  ...
  if (kind !== 'iso' && clock && +clock[2] < 60 && +clock[3] < 60) return +clock[1] * 3600 + +clock[2] * 60 + +clock[3];
